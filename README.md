...
macOS / Linux でGUI版をインストールする場合は、ターミナルで次の1行を実行してください。

```bash
curl -fsSL https://raw.githubusercontent.com/dikmri/xMosaic/main/scripts/install.sh | bash
```

インストール後はWindowsのスタートメニュー、macOSのアプリケーションフォルダ、Linuxのアプリケーションメニューから `xMosaic` を起動できます。

## インストール

上の1行インストールは、最新リリースのGUIインストーラーとPython workerを取得し、ローカル環境にセットアップします。PythonまたはFFmpegが見つからない場合は、Windowsでは `winget`、macOSでは Homebrew、Linuxでは利用可能なパッケージマネージャーで自動インストールを試みます。
